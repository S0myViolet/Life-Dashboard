...
describe('Google authorization URL', () => {
  ...
  it('adds a login hint when reconnecting a known account', () => {
    const url = new URL(
      adapter.authorize({ state: 's', codeChallenge: 'c', redirectUri: REDIRECT, loginHint: GOOGLE_EMAIL }),
    )
...
describe('Google code exchange and refresh', () => {
  it('exchanges the code with the verifier and client secret, and normalises granted scopes', async () => {
    ...
    const tokens = await adapter.exchange({ code: '4/0Asynthetic-code', codeVerifier: 'v'.repeat(43), redirectUri: REDIRECT }, c)
  ...
  it('records a partial (granular consent) grant honestly', async () => {
    const { ctx: c } = ctx(() => jsonResponse(googleExchangeResponse(now, GOOGLE_CALENDAR_ONLY_GRANTED)))
    ...
    expect(tokens.grantedScopes).toContain('https://www.googleapis.com/auth/calendar.events.readonly')
  })
  ...
  it('invalid_client → configuration failure', async () => {
    ...
    expect(await failureOf(adapter.refresh('r', c))).toMatchObject({ kind: 'config', code: 'config.invalid_client' })
  })
  ...
  it('token endpoint 429 with Retry-After, 5xx and malformed JSON', async () => {
    ...
    expect(await failureOf(adapter.refresh('r', a.ctx))).toMatchObject({ kind: 'rate_limited', retryAfterMs: 45_000 })
    const b = ctx(() => textResponse('upstream error', 502))
    expect(await failureOf(adapter.refresh('r', b.ctx))).toMatchObject({ kind: 'transient', code: 'transient.http_502' })
    const d = ctx(() => textResponse('{"access_token": ', 200))
    expect(await failureOf(adapter.refresh('r', d.ctx))).toMatchObject({ code: 'transient.malformed_response' })
  })
})
...
describe('Google identity and access checks', () => {
  it('identifies the account from the id token (no extra request)', async () => {
    ...
    expect(id).toEqual({ externalAccountId: GOOGLE_SUB, accountLabel: GOOGLE_EMAIL, accountKind: 'personal' })
    expect(calls).toHaveLength(0)
  })

  it('falls back to userinfo when the id token is for another client or expired', async () => {
    for (const bad of [{ aud: 'someone-else' }, { exp: Math.floor(now.getTime() / 1000) - 3600 }, { iss: 'https://evil.test' }]) {
      const { ctx: c, calls } = ctx(() => jsonResponse({ ...googleUserinfo, hd: 'example.org', email: 'Owner@Example.org' }))
      const id = await adapter.identify(
        { accessToken: 'a', accessTokenExpiresAt: null, refreshToken: 'r', grantedScopes: [], idToken: googleIdToken(now, bad) },
        c,
      )
      expect(id).toEqual({ externalAccountId: GOOGLE_SUB, accountLabel: 'owner@example.org', accountKind: 'work_or_school' })
  ...
  it('verifies access through the Gmail profile when Gmail was granted', async () => {
    ...
    const check = await adapter.verifyAccess('ya29.a', googleExchangeResponse(now).scope.split(' '), c)
  ...
  it('classifies Gmail API failures: 401, insufficient scope, rate limits, disabled API, 5xx', async () => {
    ...
    const cases: [number, unknown, Record<string, string>, Partial<ConnectionFailure>][] = [
      ...
      [403, googleUserRateLimit, {}, { kind: 'rate_limited', code: 'rate_limited.userratelimitexceeded' }],
      [429, googleTooManyRequests, { 'retry-after': '120' }, { kind: 'rate_limited', retryAfterMs: 120_000 }],
      [403, googleApiDisabled, {}, { kind: 'config', code: 'config.api_disabled' }],
      [403, { error: { code: 403, status: 'PERMISSION_DENIED' } }, {}, { kind: 'auth', code: 'auth.forbidden' }],
...
describe('Google revoke', () => {
  it('posts the token form-encoded and reports the outcome without throwing', async () => {
    ...
    const gone = ctx(() => jsonResponse({ error: 'invalid_token', error_description: 'Token expired or revoked' }, 400))

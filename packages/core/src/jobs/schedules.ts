...
export const JOB_SCHEDULE_DEFINITIONS: readonly JobScheduleDefinition[] = Object.freeze(
  z.array(JobScheduleDefinitionSchema).parse([
    ...
    // Brief §4 cadences. Disabled until their handlers exist (Milestones 2–3).
    {
      ...
      intervalSeconds: 900,
      enabled: true,
      note: 'Milestone 0: token refresh + access check per account; Gmail/Calendar import arrives in Milestone 2',
    },
    {
      name: 'sync.microsoft',
      kind: 'sync.microsoft',
      ...
      intervalSeconds: 900,
      enabled: true,
      note: 'Milestone 0: token refresh + access check per account; Outlook import arrives in Milestone 2',
    },
    {
      name: 'sync.lunchflow',
      kind: 'sync.lunchflow',

...
describe('schema', () => {
  it('the job_kind domain lists exactly JOB_KINDS', async () => {
    ...
    await expect(
      t.db`insert into private.jobs (kind) values ('sync.nonexistent')`,
    ).rejects.toThrow(/job_kind/)
  })
  ...
  it('anon and authenticated cannot touch the queue', async () => {
    ...
    for (const run of [
      ...
      await expect(
        run(`select * from private.claim_jobs('x', 1, 1000)`),
      ).rejects.toThrow(/permission denied/)
      await expect(
        run(`select * from private.enqueue_job('sync.rss')`),
      ).rejects.toThrow(/permission denied/)
    }
...
describe('enqueue', () => {
  it('is idempotent on dedupe_key and returns the existing job', async () => {
    const a = await svc((tx) =>
      enqueueJob(tx, { kind: 'sync.rss', dedupeKey: 'sync.rss:idem-1', payload: { feedId: 'f1' } }, T0),
    )
    const b = await svc((tx) =>
      enqueueJob(tx, { kind: 'sync.rss', dedupeKey: 'sync.rss:idem-1', payload: { feedId: 'other' } }, T0),
    )
  ...
  it('is idempotent under concurrency: 25 racing enqueues create one job', async () => {
    const results = await Promise.all(
      Array.from({ length: 25 }, () =>
        svc((tx) => enqueueJob(tx, { kind: 'briefing.morning', dedupeKey: 'briefing.morning:2026-01-01' }, T0)),
      ),
    )
...
describe('claim', () => {
  it('8 concurrent claimers over 50 jobs process every job exactly once', async () => {
    ...
    try {
      for (let i = 0; i < 50; i++) {
        await withService(db.db, (tx) => enqueueJob(tx, { kind: 'sync.rss', payload: { n: i } }, T0))
      }
      ...
      expect(rows.every((r) => r.attempts === 1 && r.leaseToken === null && r.finishedAt !== null)).toBe(true)
  ...
  it('only claims due jobs of the requested kinds', async () => {
    ...
    try {
      ...
      const got = await withService(db.db, (tx) =>
        claimJobs(tx, { workerId: 'w', limit: 10, leaseMs: 10_000, kinds: ['briefing.morning', 'sync.rss'], now: T0 }),
      )
      expect(got.map((j) => j.kind)).toEqual(['briefing.morning'])
      expect(
        await withService(db.db, (tx) => claimJobs(tx, { workerId: 'w', limit: 10, leaseMs: 10_000, kinds: [], now: T0 })),
      ).toEqual([])
      const later = await withService(db.db, (tx) =>
        claimJobs(tx, { workerId: 'w', limit: 10, leaseMs: 10_000, kinds: ['sync.rss'], now: plus(60_000) }),
      )
  ...
  it('rejects nonsense claim parameters', async () => {
    await expect(svc((tx) => claimJobs(tx, { workerId: '', limit: 1, leaseMs: 1000 }))).rejects.toThrow(/worker/)
    await expect(svc((tx) => claimJobs(tx, { workerId: 'w', limit: 0, leaseMs: 1000 }))).rejects.toThrow(/limit/)
    await expect(svc((tx) => claimJobs(tx, { workerId: 'w', limit: 1, leaseMs: 10 }))).rejects.toThrow(/lease/)
  })
})
...
describe('leases', () => {
  it('an expired lease is reclaimed by another worker; the original worker can no longer finish', async () => {
    ...
    try {
      ...
      const [a] = await withService(db.db, (tx) => claimJobs(tx, { workerId: 'A', limit: 1, leaseMs: 1_000, now: T0 }))
      expect(a?.id).toBe(job.id)
      // Still leased by A: B gets nothing.
      expect(
        await withService(db.db, (tx) => claimJobs(tx, { workerId: 'B', limit: 1, leaseMs: 1_000, now: plus(999) })),
      ).toEqual([])
      // A stalls past its lease; B reclaims with a new token.
      const [b] = await withService(db.db, (tx) => claimJobs(tx, { workerId: 'B', limit: 1, leaseMs: 30_000, now: plus(1_500) }))
      expect(b?.id).toBe(job.id)
      expect(b?.leaseToken).not.toBe(a?.leaseToken)
      expect(b?.attempts).toBe(2)
      expect(b?.leaseOwner).toBe('B')

      expect(await withService(db.db, (tx) => completeJob(tx, a!, { result: { by: 'A' }, now: plus(2_000) }))).toBe(false)
      expect(await withService(db.db, (tx) => failJob(tx, a!, { error: new Error('late'), attempt: 1, now: plus(2_000) }))).toBeNull()
      expect(await withService(db.db, (tx) => heartbeatJob(tx, a!, { leaseMs: 30_000, now: plus(2_000) }))).toBeNull()

      expect(await withService(db.db, (tx) => completeJob(tx, b!, { result: { by: 'B' }, now: plus(3_000) }))).toBe(true)
      const final = await withService(db.db, (tx) => getJob(tx, job.id))
      expect(final).toMatchObject({ status: 'succeeded', result: { by: 'B' }, attempts: 2, leaseToken: null, finishedAt: plus(3_000) })
      // A finished job cannot be completed again, even with the right token.
      expect(await withService(db.db, (tx) => completeJob(tx, b!, { now: plus(4_000) }))).toBe(false)
    } finally {
      await db.drop()
    }
  })
  ...
  it('heartbeats keep a long job leased', async () => {
    ...
    try {
      ...
      const [a] = await withService(db.db, (tx) => claimJobs(tx, { workerId: 'A', limit: 1, leaseMs: 1_000, now: T0 }))
      const expiry = await withService(db.db, (tx) => heartbeatJob(tx, a!, { leaseMs: 1_000, now: plus(800) }))
      expect(expiry).toEqual(plus(1_800))
      expect(
        await withService(db.db, (tx) => claimJobs(tx, { workerId: 'B', limit: 1, leaseMs: 1_000, now: plus(1_500) })),
      ).toEqual([])
  ...
  it('a lease that expires during the final attempt is reaped as dead', async () => {
    const db = await freshDb()
    try {
      const { job } = await withService(db.db, (tx) => enqueueJob(tx, { kind: 'sync.rss', maxAttempts: 1 }, T0))
      await withService(db.db, (tx) => claimJobs(tx, { workerId: 'A', limit: 1, leaseMs: 1_000, now: T0 }))
      expect(await withService(db.db, (tx) => reapExpiredJobs(tx, { now: plus(999) }))).toEqual([])
      expect(
        await withService(db.db, (tx) => claimJobs(tx, { workerId: 'B', limit: 1, leaseMs: 1_000, now: plus(2_000) })),
      ).toEqual([])
      const reaped = await withService(db.db, (tx) => reapExpiredJobs(tx, { kinds: ['sync.rss'], now: plus(2_000) }))
      expect(reaped.map((j) => j.id)).toEqual([job.id])
      expect(reaped[0]).toMatchObject({ status: 'dead', lastError: 'Lease expired during the final attempt', finishedAt: plus(2_000) })
    } finally {
      await db.drop()
    }
  })
...
describe('failure handling', () => {
  it('retries with deterministic backoff and becomes claimable only after run_at', async () => {
    ...
    try {
      const { job } = await withService(db.db, (tx) => enqueueJob(tx, { kind: 'sync.rss' }, T0))
      const [c1] = await withService(db.db, (tx) => claimJobs(tx, { workerId: 'A', limit: 1, leaseMs: 10_000, now: T0 }))
      const failedAt = plus(500)
      expect(await withService(db.db, (tx) => failJob(tx, c1!, { error: new Error('HTTP 503'), attempt: 1, now: failedAt }))).toBe('failed')
      const expected = computeJobRetryAt({ now: failedAt, attempt: 1, seed: job.id })
      const row = await withService(db.db, (tx) => getJob(tx, job.id))
      expect(row).toMatchObject({ status: 'failed', runAt: expected, lastError: 'HTTP 503', leaseToken: null, finishedAt: null })
      ...
      expect(await withService(db.db, (tx) => claimJobs(tx, { workerId: 'A', limit: 1, leaseMs: 10_000, now: tooEarly }))).toEqual([])
      const [c2] = await withService(db.db, (tx) => claimJobs(tx, { workerId: 'A', limit: 1, leaseMs: 10_000, now: expected }))
      expect(c2?.attempts).toBe(2)
      // The second failure waits longer (exponential).
      await withService(db.db, (tx) => failJob(tx, c2!, { error: 'again', attempt: 2, now: expected }))
  ...
  it('honours Retry-After when it is later than the backoff', async () => {
    ...
    try {
      ...
      const [c] = await withService(db.db, (tx) => claimJobs(tx, { workerId: 'A', limit: 1, leaseMs: 10_000, now: T0 }))
      ...
      const direct = await withService(db.db, async (tx) => {
        const [again] = await claimJobs(tx, { workerId: 'A', limit: 1, leaseMs: 10_000, now: retryAfter })
  ...
  it('goes dead after max attempts, or at once when not retryable', async () => {
    const db = await freshDb()
    try {
      const { job } = await withService(db.db, (tx) => enqueueJob(tx, { kind: 'sync.rss', maxAttempts: 3 }, T0))
      let now = T0
      const outcomes: (string | null)[] = []
      for (let i = 1; i <= 3; i++) {
        const [c] = await withService(db.db, (tx) => claimJobs(tx, { workerId: 'A', limit: 1, leaseMs: 10_000, now }))
        expect(c?.attempts).toBe(i)
        outcomes.push(await withService(db.db, (tx) => failJob(tx, c!, { error: `boom ${i}`, attempt: i, now })))
      ...
      expect(await withService(db.db, (tx) => claimJobs(tx, { workerId: 'A', limit: 1, leaseMs: 10_000, now: plus(10 * 86_400_000) }))).toEqual([])

      const { job: j2 } = await withService(db.db, (tx) => enqueueJob(tx, { kind: 'sync.rss' }, T0))
      const [c2] = await withService(db.db, (tx) => claimJobs(tx, { workerId: 'A', limit: 1, leaseMs: 10_000, now: T0 }))
      expect(c2?.id).toBe(j2.id)
      expect(await withService(db.db, (tx) => failJob(tx, c2!, { error: 'invalid payload', attempt: 1, retryable: false, now: T0 }))).toBe('dead')
    } finally {
      await db.drop()
    }
  })
  ...
  it('stores only a sanitised, bounded last_error', async () => {
    const db = await freshDb()
    try {
      const { job } = await withService(db.db, (tx) => enqueueJob(tx, { kind: 'sync.google', maxAttempts: 2 }, T0))
      const [c] = await withService(db.db, (tx) => claimJobs(tx, { workerId: 'A', limit: 1, leaseMs: 10_000, now: T0 }))
      const err = new Error(
      ...
      const [c2] = await withService(db.db, (tx) => claimJobs(tx, { workerId: 'A', limit: 1, leaseMs: 10_000, now: plus(7_200_000) }))
      const raw = `line1\nline2\u0007${'x'.repeat(5000)}`
      await t.db`select 1` // keep the shared pool warm
      await withService(db.db, (tx) => tx`select private.fail_job(${c2!.id}::uuid, ${c2!.leaseToken}::uuid, ${raw}, null, true, ${plus(7_200_000)}::timestamptz)`)
...
describe('schedules', () => {
  it('syncs definitions idempotently and tracks enabled_since', async () => {
    ...
    try {
      ...
      expect(first.map((s) => s.name).sort()).toEqual(JOB_SCHEDULE_DEFINITIONS.map((d) => d.name).sort())
      const morning = first.find((s) => s.name === 'briefing.morning')!
      expect(morning).toMatchObject({ cadence: 'daily_local_time', localTime: '11:00', enabled: true, enabledSince: T0 })
      expect(first.find((s) => s.name === 'sync.google')).toMatchObject({ enabled: false, enabledSince: null, intervalSeconds: 900 })

      // Re-sync later: nothing changes (enabled_since is not bumped, no write at all).
      await withService(db.db, (tx) => syncJobSchedules(tx, JOB_SCHEDULE_DEFINITIONS, plus(86_400_000)))
      ...
      const without: JobScheduleDefinition[] = JOB_SCHEDULE_DEFINITIONS.filter((d) => d.name !== 'briefing.evening')
      await withService(db.db, (tx) => syncJobSchedules(tx, without, plus(1_000)))
      expect((await withService(db.db, (tx) => listJobSchedules(tx))).find((s) => s.name === 'briefing.evening')).toMatchObject({
        enabled: false,
        enabledSince: null,
      })
  ...
  it('completing a scheduled job records last_success_at', async () => {
    ...
    try {
      await withService(db.db, (tx) => syncJobSchedules(tx, JOB_SCHEDULE_DEFINITIONS, T0))
      await withService(db.db, (tx) =>
        enqueueJob(tx, { kind: 'briefing.morning', dedupeKey: 'briefing.morning:2026-09-24', scheduleName: 'briefing.morning' }, T0),
      )
      const [c] = await withService(db.db, (tx) => claimJobs(tx, { workerId: 'A', limit: 1, leaseMs: 10_000, now: T0 }))
      await withService(db.db, (tx) => completeJob(tx, c!, { now: plus(5_000) }))
      const s = (await withService(db.db, (tx) => listJobSchedules(tx))).find((x) => x.name === 'briefing.morning')

...
const content = (c: Partial<NoteContentInput>): NoteContentInput => ({ ...EMPTY_NOTE_CONTENT, ...c })
...
describe('saving notes with optimistic concurrency', () => {
  it('creates with a device id, and a repeated create (lost response) is idempotent', async () => {
    const id = randomUUID()
    const first = await asOwner((tx) => saveNote(tx, { id, baseVersion: 0, content: content({ body: 'hello' }) }))
    expect(first.status).toBe('saved')
    if (first.status !== 'saved') return
    expect(first.note.version).toBe(1)

    const again = await asOwner((tx) => saveNote(tx, { id, baseVersion: 0, content: content({ body: 'hello' }) }))
    ...
    const other = await asOwner((tx) => saveNote(tx, { id, baseVersion: 0, content: content({ body: 'other' }) }))
  ...
  it('updates only from the current version and never silently overwrites a newer save', async () => {
    const id = randomUUID()
    await asOwner((tx) => saveNote(tx, { id, baseVersion: 0, content: content({ title: 'T', body: 'v1' }) }))
    const laptop = await asOwner((tx) =>
      saveNote(tx, { id, baseVersion: 1, content: content({ title: 'T', body: 'v2 from laptop' }) }),
    )
  ...
  it('treats a retried update whose response was lost as saved', async () => {
    ...
    const retry = await asOwner((tx) => saveNote(tx, { id, baseVersion: 1, content: content({ body: 'b' }) }))
  ...
  it('reports not_found for an edit to a deleted note (the device keeps its text)', async () => {
    ...
    expect(await asOwner((tx) => saveNote(tx, { id, baseVersion: 1, content: content({ body: 'y' }) }))).toEqual({
      status: 'not_found',
    })
  ...
  it('validates input at the boundary and stores links', async () => {
    const bad = await asOwner((tx) =>
      saveNote(tx, { id: randomUUID(), baseVersion: 0, content: content({ title: 'x'.repeat(301) }) }),
    )
    expect(bad.status).toBe('invalid')

    const [book] = await t.db<{ id: string }[]>`insert into public.books (title) values ('Dune') returning id`
    const [person] = await t.db<{ id: string }[]>`insert into public.people (name) values ('Sam') returning id`
    const id = randomUUID()
    const saved = await asOwner((tx) =>
      saveNote(tx, {
        id,
        baseVersion: 0,
        content: content({ body: 'linked', linkedDate: '2026-09-24', bookId: book!.id, personId: person!.id }),
      }),
    )
    expect(saved.status).toBe('saved')
    if (saved.status === 'saved') {
      expect(saved.note).toMatchObject({ linkedDate: '2026-09-24', bookId: book!.id, personId: person!.id })
    }
  ...
  it('strips NUL characters that Postgres text cannot store', async () => {
    const id = randomUUID()
    const r = await asOwner((tx) => saveNote(tx, { id, baseVersion: 0, content: content({ body: 'a\u0000b' }) }))
...
describe('listing', () => {
  it('lists pinned notes first, then most recently edited, with a pinned-only view', async () => {
    ...
    for (const [i, id] of ids.entries()) {
      await asOwner((tx) => saveNote(tx, { id, baseVersion: 0, content: content({ title: `List ${i}` }) }))
    }
  ...
  it('finds prefix matches in title and body, ranks title hits first and highlights them', async () => {
    ...
    await asOwner((tx) =>
      saveNote(tx, {
        id: b,
        baseVersion: 0,
        content: content({ title: 'Groceries', body: 'Remember the planning poker cards and milk.' }),
      }),
    )
    ...
    expect(notesHighlightSegments(mine[1]!.snippet).some((s) => s.match && s.text === 'planning')).toBe(true)
...
describe('link options', () => {
  it('returns [] for projects while public.projects does not exist, and reads it once it does', async () => {
    const before = await asOwner((tx) => listNoteLinkOptions(tx))
    expect(before.projects).toEqual([])
    expect(before.people.map((p) => p.label)).toContain('Sam')

    // A table with an unexpected shape must not break the transaction.
    await t.db`create table public.projects (id uuid primary key default gen_random_uuid(), title text)`
    await t.db`grant select on public.projects to authenticated`
    const odd = await asOwner(async (tx) => {
      const opts = await listNoteLinkOptions(tx)
      const [still] = await tx<{ ok: number }[]>`select 1 as ok`
      return { opts, ok: still?.ok }
    })
    expect(odd).toEqual({ opts: expect.objectContaining({ projects: [] }), ok: 1 })

    await t.db`drop table public.projects`
    await t.db`
      create table public.projects (id uuid primary key default gen_random_uuid(), name text not null,
        status text not null default 'active')
    `
    await t.db`grant select on public.projects to authenticated`
    await t.db`insert into public.projects (name) values ('Home office'), ('Old') `
    await t.db`update public.projects set status = 'done' where name = 'Old'`
    const after = await asOwner((tx) => listNoteLinkOptions(tx))
    expect(after.projects.map((p) => p.label)).toEqual(['Home office'])
    await t.db`drop table public.projects`
  })
})
...
describe('row level security', () => {
  it('hides notes from anon and from a signed-in user who is not the owner', async () => {
    const id = randomUUID()
    await asOwner((tx) => saveNote(tx, { id, baseVersion: 0, content: content({ body: 'private' }) }))

    await expect(withAnon(t.db, (tx) => tx`select id from public.notes`)).rejects.toThrow(/permission denied/)
    ...
    expect(
      await withOwner(t.db, stranger, (tx) => saveNote(tx, { id, baseVersion: 1, content: content({ body: 'x' }) })),

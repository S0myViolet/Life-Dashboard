...
beforeEach(async () => {
  session.state = {
    status: 'owner',
    session: { claims: env.owner, userId: env.owner.sub, email: env.owner.email ?? null },
  }
...
describe('GET /api/connections/[provider]/start', () => {
  it('requires the owner session', async () => {
    ...
    const out = await start(
      new NextRequest(`${APP_URL}/api/connections/google/start`),
      params('google'),
    )
  ...
  it('404s for providers without an OAuth connect flow', async () => {
    ...
    for (const p of ['lunchflow', 'whoop', 'evil']) {
      expect(
        (await start(new NextRequest(`${APP_URL}/api/connections/${p}/start`), params(p))).status,
      ).toBe(404)
      expect(
        (await callback(new NextRequest(`${APP_URL}/api/connections/${p}/callback`), params(p)))
          .status,
      ).toBe(404)
    }
  })
  ...
  it('sends the owner back with needs_setup when the provider is not configured', async () => {
    ...
    try {
      const res = await start(
        new NextRequest(`${APP_URL}/api/connections/microsoft/start`),
        params('microsoft'),
      )
...
describe('GET /api/connections/[provider]/callback', () => {
  it('completes the flow with the matching cookie, clears the cookie and redirects with a closed code', async () => {
    ...
    const res = await callback(
      new NextRequest(
        `${APP_URL}/api/connections/google/callback?state=${state}&code=synthetic-code&scope=x`,
        {
    ...
    expect(res.status).toBe(303)
    expect(res.headers.get('location')).toBe(
      `${APP_URL}/settings/connections?provider=google&result=connected`,
    )
  ...
  it('without the cookie nothing is exchanged or connected', async () => {
    ...
    expect(res.headers.get('location')).toBe(
      `${APP_URL}/settings/connections?provider=google&error=state_mismatch`,
    )
  ...
  it('provider error text never reaches the redirect', async () => {
    ...
    vi.stubGlobal(
      ...
      providerFetch(() => new Date(), {
        googleToken: () =>
          jsonResponse(
            { error: 'invalid_grant', error_description: 'Bad code owner.synthetic@gmail.com' },
            400,
          ),
      }).fetch,
    )
    ...
    expect(res.headers.get('location')).toBe(
      `${APP_URL}/settings/connections?provider=google&error=exchange_failed`,
    )
  })
})
